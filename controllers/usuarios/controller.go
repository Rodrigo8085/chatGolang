...
import (
	model "chatGolang/models/usuario"
	"database/sql"
	"net/http"
	"strconv"
...
func ObtenerTodos(db *sql.DB) echo.HandlerFunc {
	return func(c echo.Context) error {
		return c.JSON(http.StatusOK, model.Usuarios(db))
...
func Obtener(db *sql.DB) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		return c.JSON(http.StatusOK, model.Usuario(db, id))
	}
}
...
func Guardar(db *sql.DB) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		guardado, err := model.Insertar(db, usuario.Nombre, usuario.Edad, usuario.Email, usuario.Contrasenia)
...
func Actualizar(db *sql.DB) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		// mapea el Json con el modelo
		c.Bind(&usuario)
		actualizado, err := model.Actualizar(db, usuario.Nombre, usuario.Edad, usuario.Email, usuario.Contrasenia, usuario.IDUsuario)
		if err == nil {
